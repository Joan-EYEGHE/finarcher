...
<div class="max-w-md mx-auto bg-white rounded-lg shadow p-6">
    <h2 class="text-2xl font-bold mb-6">Modifier le compte</h2>

    <form method="POST" action="{{ route('portefeuilles.update', $portefeuille) }}">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="nom" class="block text-sm font-medium text-gray-700 mb-1">Nom du compte</label>
            <input type="text" name="nom" id="nom" value="{{ old('nom', $portefeuille->nom) }}" required
            ...
            <select name="devise_id" id="devise_id" required
                ...
                @foreach($devises as $devise)
                    <option value="{{ $devise->id }}"
                            {{ old('devise_id', $portefeuille->devise_id) == $devise->id ? 'selected' : '' }}>
        ...
        <div class="mb-6">
            ...
            <input type="text" name="icone" id="icone" value="{{ old('icone', $portefeuille->icone) }}"
                   class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
