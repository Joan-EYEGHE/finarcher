...
<div class="max-w-md mx-auto bg-white rounded-lg shadow p-6">
    <h2 class="text-2xl font-bold mb-6">Modifier le revenu</h2>

    <form method="POST" action="{{ route('revenus.update', $revenu) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="date" name="date_operation" id="date_operation"
                   value="{{ old('date_operation', $revenu->date_operation->format('Y-m-d')) }}" required
            ...
            <select name="portefeuille_id" id="portefeuille_id" required
                ...
                @foreach($portefeuilles as $portefeuille)
                    <option value="{{ $portefeuille->id }}"
                            {{ old('portefeuille_id', $revenu->portefeuille_id) == $portefeuille->id ? 'selected' : '' }}>
                        {{ $portefeuille->nom }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="acteur_id" id="acteur_id" required
                ...
                @foreach($acteurs as $acteur)
                    <option value="{{ $acteur->id }}"
                            {{ old('acteur_id', $revenu->acteur_id) == $acteur->id ? 'selected' : '' }}>
                        {{ $acteur->nom }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="number" name="montant" id="montant"
                   value="{{ old('montant', $revenu->montant) }}"
        ...
        <div class="mb-6">
            ...
            <input type="text" name="motif" id="motif" value="{{ old('motif', $revenu->motif) }}"
                   class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
